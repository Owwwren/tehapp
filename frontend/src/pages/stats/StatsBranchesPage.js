...
function StatsBranchesPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await api.get('/statistics/branches', { params });
      setData(res.data || []);
  ...
  const displayData = selectedBranches.length > 0
    ? data.filter(r => selectedBranches.includes(r.branch_id))
    : data;
  ...
  const totals = displayData.reduce((acc, r) => {
    ...
    acc.cancelled += r.cancelled; acc.not_orders += (r.not_orders || 0);
    acc.cassa += r.cassa; acc.fact_cassa += r.fact_cassa;
    ...
    return acc;
  }, { total: 0, completed: 0, completed_cassa: 0, cancelled_cc: 0, cancelled_bt: 0, fake: 0, rejected: 0, no_show: 0, sd: 0, cancelled: 0, not_orders: 0, cassa: 0, fact_cassa: 0, prepaid: 0, parts: 0, remainder: 0, net: 0, zero_count: 0 });
  ...
  const fullData = [
    ...
    {
      branch_id: 'summary', branch_name: 'Итого', ...totals,
  ...
  const columns = [
    { title: 'Филиал', dataIndex: 'branch_name', width: 130, render: (v, r) => r.branch_id === 'summary' ? bold(v) : v },
    { title: 'Заявок', dataIndex: 'total', width: 55, render: (v, r) => r.branch_id === 'summary' ? bold(v) : v },
    { title: 'Вып', dataIndex: 'completed', width: 45, render: (v, r) => r.branch_id === 'summary' ? bold(v) : v },
    { title: 'СД', dataIndex: 'sd', width: 35, render: (v, r) => r.branch_id === 'summary' ? bold(v) : v },
    { title: 'Отказ', dataIndex: 'rejected', width: 45, render: (v, r) => r.branch_id === 'summary' ? bold(v) : v },
    { title: 'Вброс', dataIndex: 'fake', width: 45, render: (v, r) => r.branch_id === 'summary' ? bold(v) : v },
    { title: 'ОтмКЦ', dataIndex: 'cancelled_cc', width: 50, render: (v, r) => r.branch_id === 'summary' ? bold(v) : v },
    { title: 'ОтмБТ', dataIndex: 'cancelled_bt', width: 50, render: (v, r) => r.branch_id === 'summary' ? bold(v) : v },
    { title: 'Недоезд', dataIndex: 'no_show', width: 55, render: (v, r) => r.branch_id === 'summary' ? bold(v) : v },
    { title: 'Ноль', dataIndex: 'zero_count', width: 40, render: (v, r) => r.branch_id === 'summary' ? bold(v) : v },
    ...
    { title: '% Отм', dataIndex: 'cancelled_pct', width: 50, render: (v, r) => r.branch_id === 'summary' ? bold(`${v}%`) : `${v}%` },
  ...
  return (
    <Card title="📊 Статистика — Филиалы" extra={<Button icon={<ReloadOutlined />} onClick={fetchData}>Обновить</Button>}>
      <Row gutter={[12, 12]} style={{ marginBottom: 24 }} wrap>
        ...
        {data.length > 1 && (
          <Col><div style={{ fontSize: 12, color: '#888', marginBottom: 4 }}>Филиалы (таблица)</div>
            <Select mode="multiple" value={selectedBranches} onChange={setSelectedBranches} placeholder="Все" allowClear style={{ minWidth: 200 }} maxTagCount={2}
              options={data.map(r => ({ value: r.branch_id, label: r.branch_name }))} />
          </Col>
        )}
      </Row>

      <Table dataSource={fullData} rowKey="branch_id" columns={columns} loading={loading} size="small" scroll={{ x: 1800 }} pagination={false}
        rowClassName={(r) => r.branch_id === 'summary' ? 'summary-row' : ''}
        style={{ marginBottom: 32 }} />

      <Row gutter={[16, 16]}>
        <Col span={12}><Card size="small" title="💰 Средний чек по дням"><ResponsiveContainer width="100%" height={280}><LineChart data={avgDailyData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="day" tick={{ fontSize: 11 }} /><YAxis tickFormatter={(v) => `${(v/1000).toFixed(0)}т`} /><ReTooltip formatter={(v) => `${v?.toLocaleString()} ₽`} /><Line type="monotone" dataKey="avg" stroke="#722ed1" strokeWidth={2} dot={{ r: 3 }} /></LineChart></ResponsiveContainer></Card></Col>
        <Col span={12}><Card size="small" title="📉 Динамика заявок по дням"><ResponsiveContainer width="100%" height={280}><LineChart data={dailyData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="day" tick={{ fontSize: 11 }} /><YAxis allowDecimals={false} /><ReTooltip /><Line type="monotone" dataKey="count" stroke="#1677ff" strokeWidth={2} dot={{ r: 3 }} /></LineChart></ResponsiveContainer></Card></Col>
      </Row>

      <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
        <Col span={8}><Card size="small" title="📋 Воронка заказов" style={{ minHeight: 350 }}>
                      ...
                      compositeValue > 0 ? (
                        <div style={{ width: `${Math.round(compositeValue / maxValue * 100)}%`, minWidth: 60, height: 16, borderRadius: 3, overflow: 'hidden', display: 'flex' }}>
                          {funnelBreakdown.map((b, bi) => {
                            ...
                            return bpct > 0 ? <AntTooltip key={bi} title={`${b.name}: ${b.value} шт.`}><div style={{ width: `${bpct}%`, height: '100%', backgroundColor: b.fill, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 8 }}>{b.value}</div></AntTooltip> : null;
                          })}
                        </div>
                      ) : <div style={{ width: '5%', minWidth: 30, height: 16, backgroundColor: '#eee', borderRadius: 3 }} />
                    ) : (
                      <div style={{ width: `${pct}%`, minWidth: item.value > 0 ? 30 : 0, height: 16, backgroundColor: item.fill, borderRadius: 3, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 9, fontWeight: 500 }}>{item.value > 0 ? item.value : ''}</div>
                    )}
                  </div>
                </div>
              );
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginTop: 6, flexWrap: 'wrap', fontSize: 9 }}>
            <span style={{ color: '#1677ff' }}>● Всего: {totals.total}</span><span style={{ color: '#722ed1' }}>● СД: {totals.sd}</span>
            {funnelBreakdown.map((e, i) => <span key={i} style={{ color: e.fill }}>● {e.name}: {e.value}</span>)}
            <span style={{ color: '#52c41a' }}>● Выполнено: {totals.completed}</span>
          </div>
        </Card></Col>
        <Col span={8}><Card size="small" title="🔧 Распределение по технике" style={{ minHeight: 350 }}><ResponsiveContainer width="100%" height={280}><PieChart><Pie data={techData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={90} innerRadius={40} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`} labelLine={false}>{techData.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}</Pie><ReTooltip /></PieChart></ResponsiveContainer></Card></Col>
        <Col span={8}><Card size="small" title="📊 Процент в деньги" style={{ minHeight: 350 }}><ResponsiveContainer width="100%" height={280}><PieChart><Pie data={[{ name: 'В деньги', value: moneyPct }, { name: 'Остальное', value: 100 - moneyPct }]} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={90} innerRadius={40} label={({ payload }) => payload.name === 'В деньги' ? `${moneyPct}%` : ''}><Cell fill="#52c41a" /><Cell fill="#f5222d" /></Pie><ReTooltip formatter={(v) => `${v}%`} /></PieChart></ResponsiveContainer></Card></Col>
      </Row>

      {showCompare && (
        <>
          <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
            <Col span={12}><Card size="small" title="📈 Эффективность по филиалам"><ResponsiveContainer width="100%" height={280}><BarChart data={compareData} layout="vertical"><CartesianGrid strokeDasharray="3 3" /><XAxis type="number" domain={[0, 100]} /><YAxis type="category" dataKey="name" tick={{ fontSize: 11 }} width={90} /><ReTooltip formatter={(v) => `${v}%`} /><Bar dataKey="efficiency" fill="#1677ff" radius={[0,6,6,0]} /></BarChart></ResponsiveContainer></Card></Col>
            <Col span={12}><Card size="small" title="💎 Маржа % по филиалам"><ResponsiveContainer width="100%" height={280}><BarChart data={compareData} layout="vertical"><CartesianGrid strokeDasharray="3 3" /><XAxis type="number" domain={[0, 100]} /><YAxis type="category" dataKey="name" tick={{ fontSize: 11 }} width={90} /><ReTooltip formatter={(v) => `${v}%`} /><Bar dataKey="margin" fill="#52c41a" radius={[0,6,6,0]} /></BarChart></ResponsiveContainer></Card></Col>
          </Row>
          <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
            <Col span={12}><Card size="small" title="💵 Средний чек по филиалам"><ResponsiveContainer width="100%" height={280}><BarChart data={compareData} layout="vertical"><CartesianGrid strokeDasharray="3 3" /><XAxis type="number" tickFormatter={(v) => `${(v/1000).toFixed(0)}т`} /><YAxis type="category" dataKey="name" tick={{ fontSize: 11 }} width={90} /><ReTooltip formatter={(v) => `${v?.toLocaleString()} ₽`} /><Bar dataKey="avg_completed" fill="#722ed1" radius={[0,6,6,0]} /></BarChart></ResponsiveContainer></Card></Col>
            <Col span={12}><Card size="small" title="❌ Отмены по филиалам"><ResponsiveContainer width="100%" height={280}><BarChart data={compareData} layout="vertical"><CartesianGrid strokeDasharray="3 3" /><XAxis type="number" /><YAxis type="category" dataKey="name" tick={{ fontSize: 11 }} width={90} /><ReTooltip /><Bar dataKey="cancelled" fill="#f5222d" radius={[0,6,6,0]} /></BarChart></ResponsiveContainer></Card></Col>
          </Row>
        </>
      )}
    </Card>
...
export default StatsBranchesPage;