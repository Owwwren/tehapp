...
import { Table, Card, Tag, Button, Space, Modal, Form, Input, Select, message, notification, DatePicker, InputNumber } from 'antd';
...
function StaffActivePage() {
  ...
  const handleEdit = async (user) => {
    ...
    const fields = {
      ...
      department_id: null, region_id: null, city_id: firstBranch?.city_id || null,
    };
    if (user.role?.code === 'master') {
      try {
        const res = await api.get(`/users/${user.id}/technics`);
        fields.technic_ids = res.data;
      } catch (e) {}
    }
  ...
  const handleFire = (record) => {
    Modal.confirm({
      title: 'Уволить сотрудника?', content: `${record.last_name} ${record.first_name} будет перемещён в архив.`,
      okText: 'Уволить', cancelText: 'Отмена', okType: 'danger',
      onOk: async () => {
        try { await api.put(`/users/${record.id}`, null, { params: { is_active: false } }); notification.info({ title: 'Сотрудник уволен', description: `${record.last_name} ${record.first_name} перемещён в архив.`, placement: 'topRight', duration: 4 }); refreshAll(); }
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      let user;
      if (editingUser) { await api.put(`/users/${editingUser.id}`, null, { params: userData }); user = editingUser; }
      else { const res = await api.post('/users/', null, { params: userData }); user = res.data; }
      if (user && technic_ids && values.role_id) {
        const role = allRoles.find(r => r.id === values.role_id);
        if (role?.code === 'master') {
          await api.post(`/users/${user.id}/technics`, { technic_ids });
        }
      }
      if (user && localFiles.length > 0) {
        for (const lf of localFiles) { const fd = new FormData(); fd.append('file', lf.file); await api.post(`/users/${user.id}/photos?photo_type=${lf.type}`, fd, { headers: { 'Content-Type': 'multipart/form-data' } }); }
        setLocalFiles([]);
      }
      message.success(editingUser ? 'Сотрудник обновлён' : 'Сотрудник создан');
    ...
    { title: 'Активен', dataIndex: 'is_active', key: 'is_active', render: (v) => v ? <Tag color="green">Да</Tag> : <Tag color="red">Нет</Tag> },
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          <Button size="small" type="link" danger onClick={() => handleFire(record)}>Уволить</Button>
        </Space>
      ),
    },
  ];
  ...
  return (
    <Card title="👨‍🔧 Сотрудники"
      extra={<Space><Button icon={<ReloadOutlined />} onClick={fetchUsers}>Обновить</Button>
        <Button type="primary" icon={<PlusOutlined />} onClick={() => { setEditingUser(null); form.resetFields(); setLocalFiles([]); setUserPhotos([]); setFormRegions(allRegions); setFormCities(allCities); setFormBranches(allBranches); setModalOpen(true); }}>Добавить</Button>
      </Space>}
    >
      ...
      <Modal title={editingUser ? 'Редактировать сотрудника' : 'Новый сотрудник'} open={modalOpen} onCancel={() => { setModalOpen(false); setEditingUser(null); form.resetFields(); }} footer={null}>
        <Form form={form} layout="vertical" onFinish={handleSubmit}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            <Form.Item name="password" label="Пароль" rules={editingUser ? [] : [{ required: true }]} style={{ flex: 1 }}><Input.Password placeholder={editingUser ? 'Новый пароль (не обязательно)' : 'Пароль'} /></Form.Item>
          </div>
          <Form.Item name="telegram_nick" label="Ник телеграм"><Input placeholder="@nickname" /></Form.Item>

          <div style={{ display: 'flex', gap: 12 }}>
            <Form.Item name="department_id" label="Направление" rules={editingUser ? [] : [{ required: true, message: 'Выберите направление' }]} style={{ flex: 1 }}>
              <Select placeholder="Выберите направление" allowClear options={formDepartmentOptions} onChange={handleFormDepartmentChange} />
            </Form.Item>
            <Form.Item name="region_id" label="Регион" rules={editingUser ? [] : [{ required: true, message: 'Выберите регион' }]} style={{ flex: 1 }}>
              <Select placeholder="Выберите регион" allowClear options={formRegionOptions} onChange={handleFormRegionChange} />
            </Form.Item>
          </div>
          <div style={{ display: 'flex', gap: 12 }}>
            <Form.Item name="city_id" label="Город" rules={editingUser ? [] : [{ required: true, message: 'Выберите город' }]} style={{ flex: 1 }}>
          ...
          <Form.Item noStyle shouldUpdate={(prev, cur) => prev.salary_type !== cur.salary_type}>
            {({ getFieldValue }) => {
              const salaryType = getFieldValue('salary_type') || 'rub';
              return (
                <Form.Item label="Зарплата" rules={[{ required: true, message: 'Укажите зарплату' }]}>
                  <div style={{ position: 'relative' }}>
                    <InputNumber style={{ width: '100%', paddingRight: 36 }} placeholder={salaryType === 'rub' ? '50000' : '5'} min={0} max={salaryType === 'pct' ? 100 : undefined} controls={false}
                      value={salaryType === 'rub' ? form.getFieldValue('salary') : form.getFieldValue('commission_percent')}
                      onChange={(val) => { if (salaryType === 'rub') form.setFieldsValue({ salary: val, commission_percent: undefined }); else form.setFieldsValue({ commission_percent: val, salary: undefined }); }} />
                    <span onClick={() => { form.setFieldsValue({ salary_type: salaryType === 'rub' ? 'pct' : 'rub' }); }}
                      style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', cursor: 'pointer', userSelect: 'none', fontWeight: 700, color: '#1677ff', fontSize: 18, zIndex: 2 }}>₽/%</span>
                  </div>
                </Form.Item>
              );
            }}
          </Form.Item>
          <Form.Item name="salary_type" hidden initialValue="rub"><Input /></Form.Item>
          <Form.Item name="salary" hidden><InputNumber /></Form.Item>
          <Form.Item name="commission_percent" hidden><InputNumber /></Form.Item>
          ...
          <Form.Item noStyle shouldUpdate={(prev, cur) => prev.role_id !== cur.role_id}>
            {({ getFieldValue }) => {
              const role = allRoles.find(r => r.id === getFieldValue('role_id'));
              return role?.code === 'master' ? (
          ...
          {editingUser ? (
            ...
            <div style={{ marginBottom: 16 }}><PhotoUploader entityType="user" photos={[]} onPhotosChange={() => {}} localFiles={localFiles} onLocalFilesChange={setLocalFiles} /></div>
          )}
          <Form.Item><Button type="primary" htmlType="submit" block>{editingUser ? 'Сохранить' : 'Создать'}</Button></Form.Item>
        </Form>
      </Modal>
    </Card>
  );
...
export default StaffActivePage;